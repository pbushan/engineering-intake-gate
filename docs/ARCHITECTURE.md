...
Only bounded, normalized, redacted `EvaluationEvidence` can cross the AI provider boundary. Schema 16 introduced expiring `AnalysisContextSnapshot`, attachment artifacts, reusable evaluations, and the selected-screenshot architecture. Schema 17 activates bounded screenshot bytes in that existing table. Attachment/sub-artifact keys include organization/project, attachment ID, content SHA-256, relevant processor/provider/model/schema versions, and relevant limits. Raw work items, attachment/media bytes, extracted audio, sampled frames, provider request/response bodies, and credentials remain unpersisted.
...
Supported attachment categories are UTF-8/UTF-16 text and logs, JSON, XML, CSV, validated PNG/JPEG, text/scanned PDF, XLSX, bounded DOCX text/tables, MP3/WAV/M4A, and MP4/MOV/WebM. XLS and archive/macro-enabled formats remain explicit Unsupported. Media processing uses direct no-shell ffmpeg/ffprobe/Poppler execution in a restrictive random workspace with time, byte, duration, dimension, pixel, frame, transcript, screenshot, and concurrency bounds. Processing reports explicit `processed`, `partial`, `unsupported`, `unavailable`, or `error` status plus independent media sub-stage states. Missing, unsupported, or technically unavailable evidence is context for evaluation; it does not deterministically force Intake Incomplete.

Selected video screenshots are bounded to six, stored only after deterministic relevance/deduplication, served through run/evaluation-scoped authenticated authorization, and removed by the existing evidence cleanup. They are never an ADO mutation. See [Attachment and Media Evidence](ATTACHMENT_MEDIA_EVIDENCE.md).
