...
const validationLabels: Readonly<Record<string, string>> = {
  ...
  'audit.evidenceRetentionDays': 'Reusable evidence retention days',
  'audit.maximumSelectedVideoScreenshots': 'Maximum selected video screenshots',
  'policy.id': 'Policy ID',
...
function ProfileStep({ values, criteria, processing, content, attachments, busy, dirty, validation, clearValidation, mutate, save }: ProfileStepProps) {
  ...
  return (
    <Stack spacing={2.5}>
      ...
      <SectionCard title="Audit and evidence retention" description="Audit metadata can outlive reusable normalized evidence. Selected video screenshots are reserved for a future release."><Grid container spacing={2}><Grid size={{ xs: 12, sm: 4 }}><TextField id="setup-audit.retentionDays" fullWidth required type="number" label="Audit retention days" value={valueOf(values.audit?.retentionDays)} onChange={(event) => { clearValidation('audit.retentionDays'); setNumber((next) => mutate((draft) => { if (draft.audit) draft.audit.retentionDays = next; }))(event); }} error={invalid('audit.retentionDays')} helperText={message('audit.retentionDays')} /></Grid><Grid size={{ xs: 12, sm: 4 }}><TextField id="setup-audit.evidenceRetentionDays" fullWidth required type="number" slotProps={{ htmlInput: { min: 1, max: 3650 } }} label="Reusable evidence retention days" value={valueOf(values.audit?.evidenceRetentionDays)} onChange={(event) => { clearValidation('audit.evidenceRetentionDays'); setNumber((next) => mutate((draft) => { if (draft.audit) draft.audit.evidenceRetentionDays = next; }))(event); }} error={invalid('audit.evidenceRetentionDays')} helperText={message('audit.evidenceRetentionDays') || 'Defaults to 30 days.'} /></Grid><Grid size={{ xs: 12, sm: 4 }}><TextField id="setup-audit.maximumSelectedVideoScreenshots" fullWidth required type="number" slotProps={{ htmlInput: { min: 1, max: 6 } }} label="Future key screenshots per video" value={valueOf(values.audit?.maximumSelectedVideoScreenshots)} onChange={(event) => { clearValidation('audit.maximumSelectedVideoScreenshots'); setNumber((next) => mutate((draft) => { if (draft.audit) draft.audit.maximumSelectedVideoScreenshots = next; }))(event); }} error={invalid('audit.maximumSelectedVideoScreenshots')} helperText={message('audit.maximumSelectedVideoScreenshots') || 'Reserved for PR 2; maximum 6.'} /></Grid></Grid></SectionCard>
