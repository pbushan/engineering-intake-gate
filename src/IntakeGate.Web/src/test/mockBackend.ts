...
export const runItemDetail: RunItemDetail = {
  ...
  ticketSummary: { issueSummary: 'Checkout fails after submitting payment.', expectedBehavior: 'The order should complete.', actualBehavior: 'The request returns an error.', reproductionSteps: ['Open checkout', 'Submit payment'], affectedExamples: ['Order 101'], environment: 'Production 2026.9', businessImpact: 'Customers cannot complete orders.', attachmentFindings: ['trace.log contains the failing request ID.'], investigationWarnings: [] },
  analysisContext: { snapshotId: 'context-101', schemaVersion: 'analysis-context-v2', normalizedEvidenceSchemaVersion: 'normalized-evidence-v2', evaluatedRevision: '7', sourceFields: ['System.Title', 'System.Description'], humanCommentsIncluded: 2, humanCommentsAvailable: 2, generatedCommentsExcluded: 1, attachmentCount: 1, truncationOccurred: false, redactionOccurred: true, redactionCount: 1, processingWarnings: [], expiresAtUtc: '2026-10-12T14:00:00Z' },
  attachmentProcessing: [{ attachmentId: 'attachment-1', fileName: 'trace.log', mediaType: 'text/plain', sizeBytes: 1024, status: 'processed', inspectionMode: 'text', processorIdentity: 'TextAttachmentProcessor', processorVersion: '1', cacheReused: true, truncated: false, sampled: false, pagesAvailable: null, pagesInspected: null, failureCategory: null, warnings: [], normalizedEvidencePreview: 'request failed safely' }],
  keyVideoEvidence: [{ screenshotId: 'a'.repeat(64), sourceVideoFileName: 'support-recording.mp4', timestampSeconds: 12.7, observation: 'The checkout panel shows error E-42 for order 101.', provenance: 'screen-recording-sampling-v1;key-screenshot-selection-v1', width: 1280, height: 720, sizeBytes: 2048, expiresAtUtc: '2026-10-12T14:00:00Z', artifactUrl: `/api/runs/${runSummary.runId}/items/evaluation-101/screenshots/${'a'.repeat(64)}` }],
  ai: { configuredProvider: 'fake', configuredModel: 'fake-model', providerReportedModel: 'fake-model-2026', promptVersion: 'intake-evaluator-v2', runInteractions: 1, attachmentArtifactsReused: 1, attachmentArtifactsRegenerated: 0, evaluationReused: false, originEvaluationId: null, originRunId: null, executionMode: 'normalReuseEligible' },
  plannedAdoMutation: { planId: 'plan-101', evaluatedRevision: '7', tagAdditions: ['Engineering Ready'], tagRemovals: [], exactCommentBody: 'Exact proposed comment', futureDerivedAttachmentUploads: [], contentFingerprint: 'sha256:comment' },
...
export const systemHealth: SystemHealth = {
  ...
  database: { status: 'healthy', reachable: true, currentSchemaVersion: 17, migrationCurrent: true },
...
export const onboardingDefaults: OnboardingDefaults = {
  values: {
    ...
    audit: { retentionDays: null, evidenceRetentionDays: 30, maximumSelectedVideoScreenshots: 6 }, exclusions: [], policy: null,
  },
...
export const completeDraft: OnboardingDraftState = {
  ...
  values: {
    ...
    audit: { retentionDays: 90, evidenceRetentionDays: 30, maximumSelectedVideoScreenshots: 6 },
